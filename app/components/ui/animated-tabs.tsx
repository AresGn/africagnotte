  ...
  content: (
    <div className="grid grid-cols-2 gap-4 w-full h-full">
      <img
        ...
        alt="Tab 1"
        className="rounded-lg w-full h-60 object-cover mt-0 !m-0 shadow-md border-none"
      />

      <div className="flex flex-col gap-y-2">
        <h2 className="text-2xl font-bold mb-0 text-[var(--dark-color)] mt-0 !m-0">
          Tab 1
        </h2>
...
return (
  <div className={cn("w-full max-w-lg flex flex-col gap-y-1", className)}>
    <div className="flex gap-2 flex-wrap bg-white bg-opacity-50 p-1 rounded-xl">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          onClick={() => setActiveTab(tab.id)}
          className={cn(
            "relative px-3 py-1.5 text-sm font-medium rounded-lg text-[var(--dark-color)] outline-none transition-colors"
          )}
        >
          {activeTab === tab.id && (
            <motion.div
              layoutId="active-tab"
              className="absolute inset-0 bg-[var(--primary-color)] bg-opacity-80 shadow-md !rounded-lg"
    ...
    <div className="p-4 bg-white shadow-md text-[var(--dark-color)] rounded-xl border min-h-60 h-full">
      {tabs.map(
        (tab) =>
          activeTab === tab.id && (
            <motion.div
